...
This command initializes a new working session by retrieving relevant context from the project's VectorDB (if available), presenting a context summary, and asking for the session focus. It provides continuity between sessions by leveraging semantic search over project documentation, journals, and task lists.
...
2. **If VectorDB is available:**

   a. **Retrieve recent context**
   ```bash
...
The command uses these semantic queries:
